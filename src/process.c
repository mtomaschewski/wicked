...
ni_process_reap(ni_process_t *pi)
{
	...
	if (rv == 0) {
		...
		ni_error("%s: process %d (%s) has not exited yet; now doing a blocking waitpid()",
		...
		ni_error("%s: process %d (%s) reaped in blocking waitpid after %ldm%ld.%03lds",
				__func__, pi->pid, pi->process->command,
				dif.tv_sec / 60, dif.tv_sec % 60, dif.tv_usec / 1000);
	}
